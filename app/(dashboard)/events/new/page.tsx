...
const TEMPLATES: TemplateConfig[] = [
  {
    id: 'ELEGANT',
    label: 'Казахский',
    desc: 'Тёплый ботанический стиль с золотыми акцентами и казахским колоритом',
    colors: ['#FDFAF3', '#C4943A', '#6B7C3E'],
    available: true,
  },
  {
    id: 'MODERN',
  ...
  },
  {
    id: 'ROMANTIC',
    label: 'Романтичный',
    desc: 'Нежный розово-цветочный стиль',
    colors: ['#FFF0F5', '#BE4B7A', '#F5A7C7'],
    available: false,
  },
  {
    id: 'FESTIVE',
    label: 'Праздничный',
    desc: 'Яркий и торжественный с анимацией',
    colors: ['#0D0221', '#FFE66D', '#FF6B6B'],
    available: false,
  },
]
...
function KazakhMiniPreview() {
  return (
    <div
      ...
      style={{ height: 230, backgroundColor: '#FDFAF3', fontFamily: '"Georgia", serif' }}
    >
      {/* Dot texture */}
      <div
        ...
        style={{
          opacity: 0.015,
          backgroundImage: 'radial-gradient(circle at 1px 1px, #3D2B1F 0.8px, transparent 0)',
      ...
      <div className="px-5 text-center space-y-1.5" style={{ marginTop: -2 }}>
        <p style={{ fontSize: 7.5, letterSpacing: '0.42em', color: '#C4943A', fontFamily: 'Inter, sans-serif', textTransform: 'uppercase' }}>
          Құрметті қонақтар!
        </p>
        ...
        <div className="flex gap-1.5 justify-center">
          {[{ v: '15', l: 'Күн' }, { v: '08', l: 'Сағат' }, { v: '30', l: 'Мин' }, { v: '45', l: 'Сек' }].map(({ v, l }) => (
            <div key={l} className="flex flex-col items-center gap-0.5">
              <div style={{
                width: 30, height: 28,
                border: '1px solid #C4943A40',
                background: 'linear-gradient(145deg, #C4943A12, #C4943A03)',
                borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center',
                color: '#3D2B1F', fontSize: 12, fontFamily: '"Georgia", serif',
              }}>{v}</div>
        ...
        {/* Location */}
        <p style={{ fontSize: 7.5, color: '#7A8F55', fontFamily: 'Inter, sans-serif', marginTop: 2 }}>
          📍 &ldquo;Қымбатжан&rdquo; мейрамханасы
        </p>
      </div>
    </div>
  )
...
function ComingSoonPreview({ colors }: { colors: [string, string, string] }) {
  return (
    <div className="relative w-full overflow-hidden flex flex-col items-center justify-center gap-3" style={{ height: 230 }}>
      <div
        className="absolute inset-0"
        style={{ background: `linear-gradient(145deg, ${colors[0]}, ${colors[0]}ee)` }}
      />
      {/* Vague pattern */}
      <div
        className="absolute inset-0 opacity-10"
        style={{
          backgroundImage: `radial-gradient(circle at 30% 30%, ${colors[1]}, transparent 60%), radial-gradient(circle at 70% 70%, ${colors[2]}, transparent 60%)`,
        }}
      />
      <div className="relative z-10 flex flex-col items-center gap-2">
        <div
          className="w-10 h-10 rounded-full flex items-center justify-center"
          style={{ backgroundColor: `${colors[1]}20`, border: `1px solid ${colors[1]}40` }}
        >
          <Lock size={16} style={{ color: colors[1] }} />
        </div>
        <span
          className="text-xs font-semibold px-3 py-1 rounded-full"
          style={{ backgroundColor: `${colors[1]}18`, color: colors[1], border: `1px solid ${colors[1]}35` }}
        >
          Скоро
        </span>
      </div>
...
}: { tmpl: TemplateConfig; selected: boolean; onClick: () => void }) {
  return (
    <motion.button
    ...
    >
      ...
      <div className="overflow-hidden">
        {tmpl.available ? <KazakhMiniPreview /> : <ComingSoonPreview colors={tmpl.colors} />}
      </div>
      ...
      <div className="bg-white px-4 py-3.5 flex items-center gap-3">
        ...
        {selected ? (
          ...
          <span className="text-[10px] font-medium text-gray-400 flex-shrink-0">Скоро</span>
        )}
      </div>
    ...
    <div className="flex items-center gap-2">
      {STEPS.map((label, i) => (
        <div key={label} className="flex items-center gap-2">
          ...
          <span className={`text-xs hidden sm:block ${i === current ? 'font-medium text-gray-900' : 'text-gray-400'}`}>
            {label}
          </span>
          {i < STEPS.length - 1 && (
    ...
    <div>
      ...
      <div className="max-w-xl">
        <AnimatePresence mode="wait">
          ...
          {/* ── Step 1: Template ── */}
          {step === 1 && (
            <motion.div key="template" variants={slide} initial="enter" animate="center" exit="exit" transition={{ duration: 0.22 }}
              className="space-y-5"
            >
              <div>
                <h2 className="text-base font-semibold text-gray-900">Выберите шаблон</h2>
                <p className="text-xs text-gray-400 mt-0.5">Внешний вид страницы приглашения для ваших гостей</p>
              </div>
